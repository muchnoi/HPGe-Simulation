...
void   DetectorConstruction::ConstructHPGeDetector(G4LogicalVolume* matherLogicalVolume)
{
	...
	#Cover
	cover    = G4.G4Tubs("cover", 0. *mm, shellRadius, 0.5*coverThick, sphi,	dphi)
	logCover = G4.G4LogicalVolume(cover, coverMat,"logCover",0,0,0)
	V        = G4.G4ThreeVector(0. ,0. ,detectorMove + 0.5 *shellLength+ 0.5* coverThick)
	phsCover = G4.G4PVPlacement(0, V, logCover, "physiCover",	matherLogicalVolume, false, 0, fCheckOverlaps)

	#Detector
	HPGe    = G4.G4Tubs("HPGe", 0.*mm, shellRadius, 0.5*shellLength,	sphi,	dphi)
	logHPGe	= G4.G4LogicalVolume(HPGe,vacuum,"logHPGe",0,0,0)
  V       = G4.G4ThreeVector(0. ,0. ,detectorMove)
	phsHPGe = G4.G4PVPlacement(0, V, logHPGe, "physiHPGe",	matherLogicalVolume, false, 0, fCheckOverlaps)
	...
	#mylar
	G4VSolid *mylarLayer = new G4Tubs("mylarLayer", 0.*mm, CUPThick+ crystalRadius, 0.5*mylarThick, sphi, dphi)
	...
	new G4PVPlacement(0, G4ThreeVector(0., 0.,0.5*(shellLength + mylarThick)-shellThick-endGap),	logMylar,"physiMylarLayer",	logHPGe,false,0,fCheckOverlaps)
	new G4PVPlacement(0, G4ThreeVector(0., 0.,0.5*(shellLength - CUPLength)-shellThick-endGap), logCUP,"physiCUP", logHPGe,false,0,fCheckOverlaps)
	new G4PVPlacement(0, G4ThreeVector(0., 0.,0.5*shellLength - crystalHalfLength -shellThick-endGap -CUPTopThick ), logOuterDeadLayer,"physiOuterDeadLayer", logHPGe,false,0,fCheckOverlaps)
	new G4PVPlacement(0, G4ThreeVector(0., 0.,0.5*shellLength - activeHalfLength -shellThick-endGap -CUPTopThick -outerDeadLayerThick), logActiveCrystal,"physiActiveCrystal", logHPGe,false,0,fCheckOverlaps)
	new G4PVPlacement(0,G4ThreeVector(0., 0.,0.5*(shellLength - holeDepth+holeRadius)-shellThick-endGap -CUPTopThick -(2.*crystalHalfLength -holeDepth+holeRadius)), logInnerDeadLayer,"physiInnerDeadLayer", logHPGe,false,0,fCheckOverlaps)
